...
public class DataService
{
    ...
    public DagligSkæv OpretDagligSkaev(int patientId, int laegemiddelId, Dosis[] doser, DateTime startDato, DateTime slutDato) {
        ...
        DagligSkæv dagligSkaev = new DagligSkæv(startDato, slutDato, lm, doser);
    ...
    public string AnvendOrdination(int id, Dato dato)
    {
        ...
        {
            return $"Ordination med id {id} er ikke en PN ordination";
        }

        bool succes = pnOrdination.givDosis(dato);

        if (succes)
        {
            db.SaveChanges();
            return $"Dosis givet d.{dato.dato.ToShortDateString()}";
        }
        else
        {
            return
                $"Dato {dato.dato.ToShortDateString()} er ikke inden for ordinationens gyldighedsperiode ({pnOrdination.startDen.ToShortDateString()} - {pnOrdination.slutDen.ToShortDateString()})";
        }
    }
	...
	public double GetAnbefaletDosisPerDøgn(int patientId, int laegemiddelId) {
        if (patientId <=0)
            throw new ArgumentException("ugyldigt patient ID");
        
        if (laegemiddelId <= 0)
            throw new ArgumentException("ugyldigt laegemiddel ID");
        ...
        if (laegemiddel == null)
            throw new ArgumentException("Laegemiddel findes ikke");

        double vaegt = patient.vaegt;

        if (vaegt < 25)
        {
            return laegemiddel.enhedPrKgPrDoegnLet * vaegt;
        }

        else if (vaegt <= 120)
        {
            return laegemiddel.enhedPrKgPrDoegnNormal * vaegt;
        }
        else
        {
            return laegemiddel.enhedPrKgPrDoegnTung * vaegt;
        }
